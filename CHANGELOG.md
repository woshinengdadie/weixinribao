...
### 新增
- **国内镜像升级**：代码中所有 GitHub raw / releases URL 切换到 Gitee，国内用户不再需要代理即可下载
- **自动更新安装**：用户点确认后，后端自动下载安装包 → 启动 `/VERYSILENT` 静默安装 → 退出当前程序 → 安装程序自动启动新版本
- Inno Setup 加 `CloseApplicationsFilter`，安装前自动关闭运行中的旧版本
...
### 新增
- **规则逻辑支持 extra 自定义输出**：规则逻辑中超出固定三字段的分析，LLM 输出到 `extra` 对象并在日报末尾渲染为「自定义分析」段落

## v2.1.1.0 (2026-07-21)

### 新增
- **时间范围选择升级**：手动运行和会话分析的结束时间可自定义（之前固定为"现在"），支持分析任意历史时间段
- 时间选择器上下分行排列，避免默认窗口宽度溢出

### 修复
- `build_installer.bat` 缺少 `%PYTHON%` 变量导致弹"选择应用"对话框

## v2.1.0.0 (2026-07-17)

### 新增
- **无边框窗口**：自定义 HTML 标题栏，品牌渐变一体化，原生标题栏与内容区不再割裂
- **标题栏窗口控制**：左侧绿点+程序名+版本号（可拖拽），右侧 ─ ▢ ✕ 三按钮（关闭悬停变红）
- **自动更新检查**：启动时后台比对 GitHub 版本，发现新版自动弹窗 + 打开下载页；状态栏新增「检查更新」按钮 + 红徽章
- **版本工具规范化**：`update-changelog` 加 `--message/-m` 必填参数，缺省拒绝写入空条目
- **侧栏可折叠**：图标 55px ⇄ 展开 200px + tooltip 提示，localStorage 持久化
- **UI 全面升级**：品牌渐变顶栏、四色统计卡片（蓝💬/绿✅/黄⚠️/紫📊）、欢迎区、快速操作下拉菜单

### 修复
- 检查更新接口 `NameError: name 'urllib' is not defined`（`server.py` 遗漏 `import urllib.request`）
- 已选清单回空后空状态提示永远消失
- `build_installer.bat` GBK 乱码（`chcp 936` → 65001）
- 手动运行/会话分析双击重复执行竞态窗口
- 会话名含 `\` 触发 onclick 语法错误
- hook 文件过时 hidden imports 清理

## v2.0.1.17 (2026-07-16)
...
### 新增
- 自动更新前基线版本，包含 UI 全面升级、5 个 BUG 修复
